...
## 이후 제품화 로드맵 (참고, P1 범위 아님)

P1은 위 기준으로 Python 골든/레퍼런스 구현으로서 완결된다. 이후 제품화(C# UI + 언어중립 엔진 seam → C++ 엔진 이행)는 별도 트랙이다. SPEC-XSEAM-001(이슈 #50)은 현재 `apps/xdet-console/`에 구현된 얇은 수직 슬라이스의 역사적 기준이고, 전체 알고리즘 WPF 확장의 현재 규범은 SPEC-XGUI-MASTER v0.5.1 후보와 SPEC-XSEAM-002(이슈 #58)다. 골든모델은 그 마이그레이션의 동결 오라클(정답 기준)로 존속하며 제품 배포판에는 포함되지 않는다. 상세는 `README.md`의 "제품화 로드맵 (C# UI → C++ 엔진)" 절을 참조한다. 본 절은 참고용이며, 이 문서(CLAUDE.md)의 나머지 P1 규칙(작업 우선순위·아키텍처 강제 규칙·금지 사항)은 이 절로 인해 변경되지 않는다.
