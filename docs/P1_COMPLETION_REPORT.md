...
**DQE 측정 프로토콜 문서 불일치**: `docs/XDET_measurement_protocol_v1.0.md` §1.4의 공식이 IEC 62220-1과 차원이 맞지 않는 오류가 T5(SPEC-METRICS-001) 리뷰에서 발견되어 코드(`metrics/dqe.py`)는 올바른 형태로 수정되었으나, **문서 자체는 한 번도 개정되지 않았다**(생성 이후 커밋 이력 1건 = 최초 작성). 코드와 원본 사양서가 불일치 상태로 남아있다.
