...
> 이 문서는 **SPEC이 아닌 역사적 검토 메모**다. 2026-07-10의 Python GUI 후보 결정을 기록하지만 현재 기준선 후보는 `docs/GUI_CRITERIA.md` v1.3, `SPEC-XGUI-MASTER` v0.5.1, `SPEC-XSEAM-002` v0.5.1에 있다. 현재 대상은 `apps/xdet-console/` .NET 9 WPF이며 `apps/gui/`와 napari/PySide6 선택은 구현 정본이 아니다. 사용자 승인·동결 전 구현은 허가되지 않는다.
...
*(2026-07-10 논의 시점 사용자 응답: "다시 디테일하게 작업할 예정" — 위 표는 그 시점까지의 검토 결과이며 옵션 선택은 아직 확정되지 않음)* **[확정됨 — §4.6 addendum 및 GUI_CRITERIA §2 참조: 옵션 C 단계형 채택]**

## 4.5 과거 운영 형태 결정 — Python 서브 프로젝트 (2026-07-10, 이후 대체됨)

당시 별도 프로젝트(독립 저장소) 분리 여부를 검토해 **이 저장소 내 `apps/gui/` + optional extras**를 선택했다. 이 선택은 2026-07-13 WPF 재설계에서 `apps/xdet-console/`로 대체되었으며 아래 내용은 판단 이력으로만 남긴다.

기각 사유:

1. **코어 계약 종속**: GUI는 `ProcessModule`/`run_harness()`/`CANONICAL_ORDER`/XFrame 마스크 구조를 직접 소비하는 검증 하네스의 시각화 확장이며, 소비자가 xdet 하나뿐이다. 당시 B등급 9건·T등급 11건이 미확정이라 코어 계약이 계속 변하는 시기였으므로, 분리 시 크로스 레포 버전 동기화 비용만 발생하고 이득이 없었다
...
## 4.6 범위 확정 — 옵션 C 단계형 채택 (2026-07-10 addendum)

§4의 범위는 딥리서치를 거쳐 **옵션 C(A+B 통합)를 단계형으로 확정**했다 — Phase 1(=A 단위 모듈 검증기) 완료 후 Phase 2(=B 파이프라인 비교 뷰어) 착수. 근거·품질 기준·스택 결정(napari 1순위/pyqtgraph 폴백)은 [`GUI_CRITERIA.md`](GUI_CRITERIA.md)가 단일 출처이며, 정식 SPEC은 `.moai/specs/SPEC-VIEWER-001/`로 작성 완료(plan-audit 2회 → PASS 0.94 → 최종 교차검토 READY-WITH-NOTES 반영). §6의 다음 단계 1~2는 이로써 완료되었고, 남은 절차는 기존 SPEC과 동일한 run(TDD 구현) → 리뷰 → sync다. 리서치 중 발견한 선행 코드 갭은 이슈 #15~#19로 등재되었다.

## 4.7 현재 결정 — 목적별 WPF 검증 앱 (2026-07-13 addendum)

옵션 C의 “사용+검증” 목적은 유지하되 구현은 `apps/xdet-console/`의 8개 알고리즘 그룹 탭으로 재설계했다. WPF는 typed `IXdetEngine` DTO만 호출하고 PythonNet adapter가 동결 golden의 pipeline/sequence/metric/calibration/session/tier API에 위임한다. 구체 범위·순서·인수 기준은 `SPEC-XGUI-MASTER` v0.5.1의 전수 algorithm catalog와 8개 하위 SPEC이 기준선 후보이며 사용자 승인 뒤 정본이 된다.

## 5. 과거 기술적 고려사항 (역사 기록)

- **UI 프레임워크**: Streamlit(빠른 착수, 로컬 전용) vs Jupyter 기반 뷰어(추가 서버 불필요, 대화형 탐색에 강함) vs 데스크톱(Tkinter/PyQt, 배포 단순하나 개발 비용 큼) — 트레이드오프 비교가 필요 **[확정됨 — GUI_CRITERIA §2 참조: napari 1순위 + PySide6, pyqtgraph 폴백, PyQt6 배제]**
...
1. G0 사용자 승인·v0.5.1 기준선 동결
2. `SPEC-XSEAM-002` v0.5.1 9-family generic contract 구현
3. 공통 WPF shell·RunCoordinator·artifact/run-manifest export 구현
4. `SPEC-XGUI-MASTER/plan.md` 순서로 8개 그룹 구현
5. `docs/XDET_TestSpec_v1.0.md`의 PLANNED GUI TC를 실제 자동화 증거가 있는 항목만 승격
