...
P1(알고리즘 골든모델)은 위 기준으로 완결된 상태다. 이후 제품화(C# UI + 언어중립 엔진 seam → C++ 엔진 이행)는 별도 트랙으로 계획되어 있으며, 아래 "제품화 로드맵 (C# UI → C++ 엔진)" 절 및 SPEC-XSEAM-001(이슈 #50)에서 다룬다.
...
P1(알고리즘 골든모델)의 완료는 "정확한 참조 구현"의 완결이지 제품 출하를 뜻하지 않는다. 제품화는 별도 트랙으로 SPEC-XSEAM-001(이슈 #50)에 계획되어 있으며, 이 절은 그 로드맵을 요약한다.
...
|---|---|---|---|
| **Stage 1 — P1.5** (SPEC-XSEAM-001, 이슈 #50) | 언어중립 엔진 seam(`IXdetEngine` 인터페이스 + `(dtype,shape,buffer)` DTO) + C# UI 스켈레톤(WPF + .NET 8 + ScottPlot) + pythonnet 인-프로세스 어댑터 | C# UI가 신설되어 seam을 거쳐 **실제 Python 골든을 그대로 호출**한다(얇은 수직 슬라이스: offset 모듈 + MTF 지표). 알고리즘 코드는 무변경 | 골든 동일성(비주얼 XDET-TC-021) — P1.5는 순수 트랜스포트이므로 관측 delta **정확히 0** 기대. SPEC-VIEWER-001의 [HARD] 원칙(C-09 지표 자체 계산 0 / C-11 단방향 소비 / C-20 읽기 전용)을 C#으로 계승 |
...
- 계획·사양·인수기준: SPEC-XSEAM-001(이슈 #50) — `.moai/specs/SPEC-XSEAM-001/`(spec.md / plan.md / acceptance.md)
