...
| 1순위 UI | **napari**(임베디드) + magicgui 도크 + Qt 바인딩 **PySide6**(LGPL) | GUI_CRITERIA §2.1 (float32 W/L·GPU 줌/팬·레이어 마스크·호버 값 내장) |
| 폴백 UI | **pyqtgraph**(PySide6 위, `ImageView` W/L·LUT·ROI) | GUI_CRITERIA §2.1 (Qt+pytest-qt 공유로 피벗 비용 낮음) |
| 배제 | **PyQt6**(GPL/상용), Streamlit/Panel/DearPyGui 등 | GUI_CRITERIA §2.1/§2.2 |
| 시험 프레임워크 | pytest + pytest-qt, `QT_QPA_PLATFORM=offscreen`; napari `make_napari_viewer` / 폴백 qtbot | C-14/C-15 |
...
### napari→pyqtgraph 폴백 결정 (REQ-VIEW-SPIKE HOW)

스파이크(Phase 0)에서 SG-1(호버 float32 원값)·SG-2(W/L 응답 `[T]` 100ms)·SG-3(콜드 스타트 `[T]` 10s)을 napari로 실측한다. **셋 중 하나라도 미충족이면 pyqtgraph 폴백**으로 전환한다 — 단일 순서 결정(택일·"또는" 없음). 두 스택은 Qt+pytest-qt를 공유하므로 이후 Phase의 로직 레벨 테스트(레이어 생성·수치 적용·프로브)는 백엔드 추상화 뒤에서 재사용 가능하다.
...
apps/gui/
  __init__.py
  app.py              # napari 임베디드 뷰어 셸 + magicgui 도크 (폴백: PySide6+pyqtgraph 셸)
  backend.py          # napari/pyqtgraph 백엔드 추상화 (스파이크 결과로 선택)
