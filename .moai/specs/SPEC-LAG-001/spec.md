...
- **v0.2.0 (2026-07-09)** — 구현 완료(status: implemented). 커밋 3d74e8e(lag/sequence/lag_irf) + 리뷰 결함 10건 수정. 181 passed / 12 skipped(2회 동일), 계약 5건 KEPT. 확정 사항:
  - FB confirm 실패는 FBTriggerError로 명시 실패(SWR-404 핸드셰이크 강제).
  - IRF 피팅은 수렴·잔차 품질 게이트(rms_residual_tol [T]) 통과 시에만 CalibSet(LAG) 방출, 품질 지표는 provenance 기록.
  - 검증 모드에서 pixel_f64는 독립 float64 상태 재귀로 구동(TC-021 경로 독립성).
  - XDET-TC-005는 상대 감소 + EV-104 ghost 절대 임계(주입) 이중 게이트 — 종단 비가시는 FB/실패널 통합 후.
  - CalibSet(LAG) payload 키는 common/calibset 단일 출처.
