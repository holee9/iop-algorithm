...
- **v0.2.0 (2026-07-09)** — 구현 완료(status: implemented). 커밋 830d53d(grid 모듈 구현) + 리뷰 결함 5건 수정. 331 passed / 5 skipped(2회 동일), 계약 5건 KEPT. 확정:
  - 방향 선택은 실제 피크 보유 축만 비교(배경잡음 최댓값이 진짜 피크를 앞지르지 못함).
  - Nyquist 빈은 로컬 최댓값 후보 포함 + 배경 추정 mirror 모드(edge replication 결함 제거).
  - 접힘 고조파는 로컬 최댓값 요구 없이 D_th 직접 검사(문서화된 동작 실제 구현).
  - EV-102 게이트 대리 notch_gain_1d는 실제 notch와 동일한 2-term 가우시안으로 단일화.
