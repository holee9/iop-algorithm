...
- **v0.2.1 (2026-07-09)** — SPEC-LNSG-001 plan-audit iteration 1의 결정 2 상류 동기화(cross-spec): REQ-CORR-OFFSET-4(Event-Driven) 신설 — offset 모듈이 감산 전 I_raw ≥ S_th(`raw_saturation_threshold`, [B], offset Params) 화소에 SATURATION 플래그를 설정한다. raw 포화 검출의 소재를 I_raw를 입력 프레임으로 받는 유일 단계인 offset으로 확정(T3 포화 모듈 SPEC-LNSG-001은 offset(raw) ∪ gain 클램프 누적 마스크를 소비). Scenario 1을 raw 포화 검출로 확장하여 AC 부여, 「결정 필요/확인 사항」 3을 OFFSET-4 반영으로 정정, DoD offset 항목에 raw 검출 추가. **status: implemented 유지** — REQ-CORR-OFFSET-4는 T3(SPEC-LNSG-001) run 단계에서 구현될 후속 요구이며, 본 SPEC의 기구현물(커밋 4451875·daa442e)에는 아직 미반영이다.
...
- **REQ-CORR-OFFSET-4 (Event-Driven)** — WHEN offset 모듈이 입력 프레임을 처리하면, THEN 감산 전에 I_raw ≥ S_th(raw_saturation_threshold, [B], Params) 픽셀에 SATURATION 마스크 플래그를 설정해야 한다(SWR-601 raw 포화 검출 — SPEC-LNSG-001 결정 2 이관). S_th는 offset 단계 Params 키 `raw_saturation_threshold`(TBD-[B], 부록 A; 선량 계단에서 응답 포화점 실측, 통상 full-scale의 ~98%)에 단일 소재하며, offset은 I_raw를 입력 프레임으로 받는 유일한 단계이므로 raw 포화 검출의 소재가 된다(하류 gain 클램프 SATURATION과 마스크 스택에 누적/합집합되어 T3 포화 모듈 SPEC-LNSG-001이 소비). 이 요구는 T3(SPEC-LNSG-001) run 단계에서 구현될 후속 요구이다.
...
3. **SWR-602 포화 복원 금지의 적용 경계** — REQ-CORR-OFFSET-4(v0.2.1) 신설로 offset 단계가 감산 전 raw 포화(I_raw ≥ S_th)를 SATURATION으로 설정하므로, defect 보간 시점에 raw 포화 SATURATION 마스크가 이미 존재할 수 있다(종전 "defect 시점 SATURATION 미설정" 가정은 폐기). 단 defect 모듈은 DEFECT 플래그·class_map 화소만 보간하므로 SATURATION 전용(비DEFECT) 화소는 보간 대상이 아니며, SWR-602 복원 금지의 직접 관심사는 여전히 포화 모듈(T3)이다. defect 보간의 유사 안전장치는 SWR-303 정상 이웃 전제(REQ-CORR-DEFECT-5)이다. 확인: 캘리브레이션 결함 집합(DEFECT)과 장면 포화 집합(SATURATION)의 교집합 화소 처리 우선순위(현 계약: defect는 DEFECT만 보간, SATURATION 소비는 T3 포화 모듈)가 유효한지.
