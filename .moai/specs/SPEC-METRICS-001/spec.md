...
- **v0.2.1 (2026-07-09)** — NPS-3 본문을 IEC 형태로 동기화(plan-audit SPEC-CORR-001 D6).
- **v0.2.0 (2026-07-09)** — 구현 완료(status: implemented). 커밋 bab37f5(엔진 구현) + 리뷰 결함 10건 수정. 90 passed / 21 skipped, 레이어링 계약 4건 KEPT. 구현 중 확정 사항:
  - **⚠ DQE 공식 정정**: 측정프로토콜 v1.0 §1.4의 `DQE = MTF²·q·Ka/NPS`는 IEC 62220-1과 상충(차원 역전) — 엔진은 IEC 형태 `DQE = MTF²/(q·Ka·NNPS)`로 구현하고 @MX:WARN 플래그. **프로토콜 문서 v1.1 개정 필요** (이슈 #2 기록).
  - XDET-TC-001~005·018은 TestSpec VV 매핑상 T2/T4/T9 보정 WP의 DoD — pytest skeleton으로 유지하고, T1 지표 산출부 검증은 tests/metrics/* 자체 테스트로 커버(릴리스 게이트 무결성).
  - 신규 [P] 파라미터 4종(nps_central_frac, line_noise_sig_factor, lag_plateau_frac, lag_settle_frac) — SWR 부록 A 레지스터 등재 필요.
...
- **REQ-METRICS-NPS-3 (Ubiquitous)** — DQE(f)는 DQE(f) = MTF²(f) / (q · Ka · NNPS(f))로 산출되어야 하며(프로토콜 v1.0 §1.4 원문은 차원 역전 — HISTORY v0.2.0 참조), q(RQA5 광자 fluence 계수)와 Ka(검출기면 air kerma)는 CalibSet/Params로 주입되어야 한다(하드코딩 금지, REQ-METRICS-CORE-4).
